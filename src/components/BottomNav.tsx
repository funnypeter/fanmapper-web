...
const NAV_ITEMS: NavItem[] = [
  {
    ...
    label: "Home",
    icon: (active) => (
      <svg className="w-7 h-7" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth={active ? 0 : 1.8} viewBox="0 0 24 24">
      ...
      <div className="max-w-lg mx-auto flex items-center justify-around py-2 px-4">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              {item.icon(isActive)}
              <span className={`text-xs font-medium ${isActive ? "text-primary-light" : ""}`}>
                {item.label}
              </span>
              {isActive && (
